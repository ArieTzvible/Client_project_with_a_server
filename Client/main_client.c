...
int main()
{
	...
	file_name = temp_file_name;//Getting the filename for the pointer

	while (!(debtFile = fopen(file_name, "r"))) {//Opening for reading only. 
		if (file_name_dynamic) free(file_name);
	...
	sending_line_to_a_server(file_name);
